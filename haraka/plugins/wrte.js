...
exports.wrte_user_exists = function (next, connection, params) {
    ...
    server.notes.users.findOne({ username : rcpt.user }, { fields : { username : 1, "emails.address" : 1, "emails.verified" : 1, "services.stripe" : 1, "services.btc" : 1, active : 1, amount : 1, currency : 1 } } , function(err, user) {
        if(err) {
        ...
        if (user && user.emails[0] && user.emails[0].address) {
            ...
            if( user.amount > 0 && (!user.services || (!user.services.stripe && !user.services.btc) )) {
